...
const myphotosHome = async (req, res)=>{
	res.redirect("/myphotos/1");
};

const myphotosPage = async (req, res)=>{
	let conn;
	const photoLimit = 3;
	const privacy = 1;
	...
	try {
	  ...
	  res.render("myphotos", {galleryData: galleryData, imagehref: "/gallery/thumbs/", links: galleryLinks});
	}
	catch(err) {
		console.log(err);
		res.render("myphotos", {galleryData: [], imagehref: "/gallery/thumbs/", links: ""});
	}
	finally {
		if(conn){
			await conn.end();
